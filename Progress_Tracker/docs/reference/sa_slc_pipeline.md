# South Africa SLC Data-Creation Pipeline

This document describes the implementation in:

`Data_Creation/process_sa_slc_targets.py`

The pipeline processes selected South Africa target matches through a Sentinel-1 SLC SNAP workflow and writes scene-level single-band GeoTIFF products. The default output geometry is SNAP-native terrain-corrected WGS84 resolution, not the older 10 m UTM grid.
...
2. Reads point coordinates from `Data_Creation/meria_sa_plastic_s1_slc/MERIA_SA_plastic_points.csv`.
